...
class ColorGaussianModel:
    ...
    def __init__(self, sh_degree : int, num_timestamp: int):
        ...
        self.semantic_dim = 8
        self.geometry_feature_dim = 8
        self.camera_embedding_dim = 16

        self.appearance_embedding = nn.Embedding(num_timestamp, self.camera_embedding_dim).cuda()
        self.rgb_head = MLP(
            # in_dims=self.geometry_feature_dim + self.camera_embedding_dim + self.direction_encoding.n_output_dims,
            in_dims=self.geometry_feature_dim + self.camera_embedding_dim,
            # in_dims=self.geometry_feature_dim,
            # in_dims=self.geometry_feature_dim + self.camera_embedding_dim +3, # concat direction
            out_dims=3,
            num_layers=3,
            hidden_dims=16,
            skip_connections=[1]).cuda()
    ...
    def capture(self):
        return (
            ...
            self.appearance_embedding.state_dict(),
    ...
    def restore(self, model_args, training_args):
        ...
        self.send_to_gpui_cnt = torch.zeros(
            (self.get_xyz.shape[0], shard_world_size),
            dtype=torch.int,
            device="cuda",
        )
        self.denom = denom
        ...
        self.appearance_embedding.load_state_dict(appearance_embedding_dict)
        self.rgb_head.load_state_dict(rgb_head_dict)
    ...
    @property
    def get_appearance_embedding(self):
        return self.appearance_embedding
    
    @property
    def get_rgb_head(self):
        return self.rgb_head
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self._semantic_feature = torch.zeros(fused_point_cloud.shape[0], self.semantic_dim).float().cuda()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self._semantic_feature], 'lr': training_args.semantic_lr, "name": "semantic_feature"},
        ]
        l2 =[
            {'params': list(self.appearance_embedding.parameters()), 'lr': training_args.embedding_lr, "name": "appearance_embedding"},
            {'params': list(self.rgb_head.parameters()), 'lr': training_args.rgbhead_lr, "name": "rgb_head"}
        ]

        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
        # self.optimizer = torch.optim.SGD(l, lr=0.0, momentum=0.1)
        self.optimizer2 = torch.optim.Adam(l2, lr= 0.0005, eps=1e-15)
        ...
        num_iters = training_args.iterations
        scheduler_milestones = [
            num_iters // 2,
            num_iters * 3 // 4,
            num_iters * 9 // 10]
        self.scheduler2 = torch.optim.lr_scheduler.ChainedScheduler(
            [
                # warmup
                torch.optim.lr_scheduler.LinearLR(
                    self.optimizer2, start_factor=0.01, total_iters=num_iters // 10  # We might need to modify this value
                ),
                # Linear decay
    ...
    def save_ply(
        ...
        args = utils.get_args()
        _xyz = _semantic_feature = _opacity = _scaling = _rotation = None
        ...
        semantic_feature = _semantic_feature.detach().cpu().numpy()
        ...
        attributes = np.concatenate(
            (xyz, normals, opacities, scale, rotation), axis=1
        )
    ...
    def replace_tensors_to_optimizer(self, inds):
        tensors_dict = {
            "xyz": self._xyz,
    ...
    def get_destination_1(self, world_size):
        # norm p=0
        ind = []
        dist_num = self.get_xyz.shape[0] // world_size
        for i in range(world_size):
            if i + 1 == world_size:
                dist_num = self.get_xyz.shape[0] - dist_num * i
            ind = ind + [i] * dist_num
        random.shuffle(ind)
        return torch.tensor(ind, device="cuda", dtype=torch.int)
        # return torch.randint(0, world_size, (self.get_xyz.shape[0],), device="cuda")

    def get_destination_2(self, world_size, send_to_gpui_cnt):
        assert (
            world_size == send_to_gpui_cnt.shape[1]
        ), "world_size != send_to_gpui_cnt.shape[1]"
        destination = torch.argmax(send_to_gpui_cnt, dim=1)
        return destination

    def get_destination_3(self, world_size, send_to_gpui_cnt):
        assert (
            world_size == send_to_gpui_cnt.shape[1]
        ), "world_size != send_to_gpui_cnt.shape[1]"
        dist_num = self.get_xyz.shape[0] // world_size
        destination = torch.zeros(
            (self.get_xyz.shape[0]), device="cuda", dtype=torch.int
        )
        residual = send_to_gpui_cnt.max(dim=1)[0].unsqueeze(-1) - send_to_gpui_cnt
        maximum = send_to_gpui_cnt.max()
        for i in range(world_size):
            ind = torch.argsort(residual[:, i])
            if i + 1 == world_size:
                dist_num = self.get_xyz.shape[0] - dist_num * i
            destination[ind[:dist_num]] = i
            residual[ind[:dist_num], i + 1 :] = maximum + 1

        return destination

    def get_destination_4(self, world_size, send_to_gpui_cnt):
        log_file = utils.get_log_file()
        assert (
            world_size == send_to_gpui_cnt.shape[1]
        ), "world_size != send_to_gpui_cnt.shape[1]"
        ...
        residual = send_to_gpui_cnt - send_to_gpui_cnt.max(dim=1)[0].unsqueeze(-1)
        maximum = send_to_gpui_cnt.max()
        for i in range(world_size):
            ind = torch.argsort(
                residual[:, [j for j in range(world_size) if j != i]].max(dim=1)[0]
                - residual[:, i]
            )
            if i + 1 == world_size:
                dist_num = self.get_xyz.shape[0] - dist_num * i
            destination[ind[:dist_num]] = i
            residual[ind[:dist_num], i] = 2 * (maximum + 1)
        return destination
    ...
    def need_redistribute_gaussians(self, group):
        ...
        # if args.redistribute_gaussians_mode == "smart_placement":
        #     return True # always redistribute for smart_placement. TODO: add a threshold.
    ...
    def update_gaussian2gpuid_cnt(self, gpuid2gaussian_cnt):
        assert (
            gpuid2gaussian_cnt.shape == self.send_to_gpui_cnt.shape
        ), f"shape mismatch {gpuid2gaussian_cnt.shape} != {self.send_to_gpui_cnt.shape}"
        self.send_to_gpui_cnt += gpuid2gaussian_cnt
    ...
    def redistribute_gaussians(self):
        ...
        elif args.redistribute_gaussians_mode == "smart_placement":
            destination = self.get_destination_3(
                comm_group_for_redistribution.size(), self.send_to_gpui_cnt
            )
        else:
            raise ValueError(
    ...
    def relocate_gs(self, min_opacity=0.005):
        ...
        (
            ...
            self.send_to_gpui_cnt[dead_indices],
        ) = self._update_params(sampled_idxs, ratio=ratio)

        self._opacity[sampled_idxs] = self._opacity[dead_indices]
        self._scaling[sampled_idxs] = self._scaling[dead_indices]
        # Update splats and optimizers
        self.replace_tensors_to_optimizer(sampled_idxs)
        torch.cuda.empty_cache()
    ...
    def add_new_gs(self, cap_max):
        ...
        self._opacity[sampled_idxs] = new_opacity
        self._scaling[sampled_idxs] = new_scaling
        ...
        self.replace_tensors_to_optimizer(inds=sampled_idxs)
        torch.cuda.empty_cache()
    ...
    def add_noise(self, lr):
        ...
        # gsplat says that there is no need for Quaternions to be normalized
        rotation = self._rotation
        xyz = self.get_xyz
        actual_covariance, _ = quat_scale_to_covar_preci(
            rotation, scaling, compute_covar=True, compute_preci=False, triu=False
        )
