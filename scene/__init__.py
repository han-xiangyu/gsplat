...
class Scene:
    ...
    def __init__(
        ...
        (train_cameras,
         ...
         self.whether_test_cameras_locally_loaded) = self.prefilter_cameras(
                scene_info.train_cameras,
                scene_info.test_cameras,
                args)

        # load train cameras
        self.train_cameras = cameraList_from_camInfos(train_cameras, args, self.whether_train_cameras_locally_loaded)
        ...
        if args.eval:
            ...
            self.test_cameras = cameraList_from_camInfos(test_cameras, args, self.whether_test_cameras_locally_loaded)
            # output the number of cameras in the training set and image size to the log file
            log_file.write(
    ...
    def prefilter_cameras(self, train_cameras, test_cameras, args):

        if args.num_train_cameras >= 0:
            train_cameras = train_cameras[: args.num_train_cameras]
        else:
            train_cameras = train_cameras
        
        if args.eval:
            if args.num_test_cameras >= 0:
                test_cameras = test_cameras[: args.num_test_cameras]
            else:
                test_cameras = test_cameras
        else:
            test_cameras = []

        (whether_train_cameras_locally_loaded, 
         whether_test_cameras_locally_loaded) = self.whether_cameras_are_locally_loaded(args, 
                                                    train_cameras, test_cameras) # len(self.train_cameras_is_locally_loaded) == len(self.train_cameras)
        
        return train_cameras, test_cameras, whether_train_cameras_locally_loaded, whether_test_cameras_locally_loaded

    def whether_cameras_are_locally_loaded(self, args, train_cameras, test_cameras):
        if not args.distributed_dataset_storage:
            assert not args.local_sampling, "local_sampling depends on distributed_dataset_storage. "
            train_cameras_are_locally_loaded = [True for i in range(len(train_cameras))]
            test_cameras_are_locally_loaded = [True for i in range(len(test_cameras))]

        if args.distributed_dataset_storage and not args.local_sampling:
            train_cameras_are_locally_loaded = [True for i in range(len(train_cameras))]
            test_cameras_are_locally_loaded = [True for i in range(len(test_cameras))]

        if args.distributed_dataset_storage and args.local_sampling:
            if not args.smart_image_placement:
                train_cameras_are_locally_loaded = [
                        utils.DEFAULT_GROUP.rank() == i % utils.DEFAULT_GROUP.size()
                        for i in range(len(train_cameras))]
                test_cameras_are_locally_loaded = [
                        utils.DEFAULT_GROUP.rank() == i % utils.DEFAULT_GROUP.size()
                        for i in range(len(test_cameras))]

            if args.smart_image_placement:
                ImageId2GroupId_train, ImageId2GroupId_test = aerial_image_placement_k_means(args, train_cameras, test_cameras)
                train_cameras_are_locally_loaded = [utils.DEFAULT_GROUP.rank() == ImageId2GroupId_train[i] for i in range(len(train_cameras))]
                test_cameras_are_locally_loaded = [utils.DEFAULT_GROUP.rank() == ImageId2GroupId_test[i] for i in range(len(test_cameras))]
        
        return train_cameras_are_locally_loaded, test_cameras_are_locally_loaded
