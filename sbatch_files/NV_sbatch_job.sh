
job_name="3DfoundationModel6000"
base_logdir="/lustre/fsw/portfolios/nvr/users/ymingli/gaussian/logs/spatial05_frames6000_per_8M_pts"


# Train
for i in {1..1}; do
    submit_job --gpu 8 --cpu 24 --nodes 1 --partition=grizzly,polar,polar3,polar4 --account=nvr_av_end2endav \
                    --image=/lustre/fsw/portfolios/nvr/users/ymingli/dockers/cu118.sqsh  \
                    --mounts=/lustre/:/lustre/,/lustre/fsw/portfolios/nvr/users/ymingli/miniconda3:/home/ymingli/miniconda3 \
                    --duration 4 \
                    --dependency=singleton \
                    --name $job_name \
                    --logdir ${base_logdir}/run_${i} \
                    --notimestamp \
                    --exclusive \
                    --command  "bash /lustre/fsw/portfolios/nvr/users/ymingli/gaussian/code/citygs/sbatch_files/NV_train.sh"
done

# --copy_user_code /lustre/fsw/portfolios/nvr/users/ymingli/gaussian/code/citygs:code:[*] \

# job_name="3DfoundationModelRender6000"
# base_logdir="/lustre/fsw/portfolios/nvr/users/ymingli/gaussian/logs/citygs_partial_6000_full_render"
# # Render

# submit_job --gpu 1 --cpu 48 --nodes 1 --partition=grizzly,polar,polar3,polar4 --account=nvr_av_end2endav \
#                 --image=/lustre/fsw/portfolios/nvr/users/ymingli/dockers/cu118.sqsh  \
#                 --mounts=/lustre/:/lustre/,/lustre/fsw/portfolios/nvr/users/ymingli/miniconda3:/home/ymingli/miniconda3 \
#                 --duration 4 \
#                 --dependency=singleton \
#                 --name $job_name \
#                 --logdir $base_logdir \
#                 --notimestamp \
#                 --command  "bash /lustre/fsw/portfolios/nvr/users/ymingli/gaussian/code/citygs/sbatch_files/NV_render.sh"

