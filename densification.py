...
def mcmc_densification(iteration, scene, gaussians):
    args = utils.get_args()
    timers = utils.get_timers()
    log_file = utils.get_log_file()
    ...
    if not args.disable_auto_densification and iteration <= args.densify_until_iter:
        ...
        if iteration > args.densify_from_iter and utils.check_update_at_this_iter(
            ...
            timers.start("add_and_relocate")
            gaussians.relocate_gs()
            # num_relocate = gaussians.relocate_gs()
            # log_file.write("num_relocate: {}\n".format(num_relocate))
            gaussians.add_new_gs(cap_max=args.cap_max)
            timers.stop("add_and_relocate")
