...
def training(dataset_args, opt_args, pipe_args, args, log_file, WORLD_SIZE):
    ...
    for iteration in range(
        ...
        if iteration // args.bsz % 30 == 0:
            progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}" ,"Gaussian count":f"{gaussians.get_xyz.shape[0]}"})
        ...
        if args.local_sampling:
            ...
            batched_all_cameras_idx = batched_all_cameras_idx.cpu().numpy().flatten()
        ...
        if args.mcmc:
            loss_sum += args.opacity_reg * torch.abs(gaussians.get_opacity).mean()
            loss_sum += args.scale_reg * torch.abs(gaussians.get_scaling).mean()
        ...
        with torch.no_grad():
            # Update gaussian2gpuid
            if "gaussian2gpuid_cnt" in batched_screenspace_pkg:
                gaussians.update_gaussian2gpuid_cnt(
                    batched_screenspace_pkg["gaussian2gpuid_cnt"]
                )
            ...
            # Densification
            if args.mcmc:
                mcmc_densification(iteration, scene, gaussians)
    ...
    if len(testing_iterations) > 0 and utils.check_update_at_this_iter(
        ...
        validation_configs = (
                ...
                "num_cameras": len(scene.getTestCameras()) if scene.getTestCameras() else 0},
            {
                ...
                "num_cameras": max(
                    len(scene.getTrainCameras()) // args.llffhold, args.bsz
                ),
            },
        )
            ...
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx in range(1, num_cameras + 1, args.bsz):
                    ...
                    if args.local_sampling:
                        ...
                        batched_all_cameras_idx = (
                            batched_all_cameras_idx.cpu().numpy().flatten()
                        )
