...
def main(local_rank: int, world_rank: int, world_size: int, cfg: Config):
    ...
    if world_size < 2:
        if world_rank == 0:
            print(
                "Error: This benchmark requires at least 2 processes (world_size >= 2)."
            )
        return
    ...
    if world_rank == 0:
        ...
        print(
            f"Tensor Size: {cfg.tensor_size_mb} MB ({tensor_size_bytes / 1e9:.3f} GB)"
        )
    ...
    if world_rank == 0:
        ...
        print("\n------------------ RESULTS ------------------")
        print(
            f"Total time for {cfg.num_iterations} iterations: {elapsed_time_s:.4f} seconds"
        )
        ...
        print("-------------------------------------------")
        print(
            "Note: This is an approximation of the effective bandwidth for the all_reduce operation."
        )
