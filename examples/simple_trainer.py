...
class Config:
    ...
    ply_steps: List[int] = field(
        default_factory=lambda: [40_000, 80_000, 120_000, 160_000, 200_000]
    )
    # Steps to save the model as ply
...
def create_splats_with_optimizers(
    ...
    print(
        f"[PROFILING] Rank {world_rank}: Starting KNN for {points_shard.shape[0]} points..."
    )
    knn_start_time = time.time()
    dist2_avg = (knn(points_shard, 4)[:, 1:] ** 2).mean(dim=-1)
    knn_end_time = time.time()
    print(
        f"✅ [PROFILING] Rank {world_rank}: KNN took {knn_end_time - knn_start_time:.4f} seconds."
    )
...
class Runner:
    ...
    def __init__(
        ...
        print(
            f"✅ [PROFILING] Parser initialization took: {parser_end_time - parser_start_time:.4f} seconds."
        )
