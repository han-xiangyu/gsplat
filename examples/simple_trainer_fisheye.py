...
import viser
import yaml
from datasets_utils.colmap import Dataset, Parser
...
class Runner:
    ...
    def _get_camera_model_from_name(self, image_name: str) -> str:
        """根据图像文件名中包含的 'channel_{i}' 来决定相机模型。"""
        try:
            # 尝试从文件名中解析 'channel_X'
            ...
            for i, part in enumerate(parts):
                if part == 'channel' and i + 1 < len(parts):
                    # "channel" 后面的部分就是索引
                    channel_idx = int(parts[i+1])
                    break
            
            if 4 <= channel_idx <= 6:
                return "fisheye"
            else:
                # 默认或 channel 为 1, 2, 3 的情况
                return "pinhole"
        except (ValueError, IndexError):
            # 如果解析失败（比如文件名格式不符），返回默认的 pinhole 模型
            return "pinhole"
    ...
    def train(self):
        ...
        if resume_path and os.path.exists(resume_path):
            ...
            self.optimizers = create_optimizers_for_splats(self.splats, cfg, self.scene_scale, world_size)
        ...
        for step in pbar:
            ...
            image_name = self.trainset.parser.image_names[image_ids.item()]
            camera_model_to_use = self._get_camera_model_from_name(image_name)
            ...
            renders, alphas, info = self.rasterize_splats(
                ...
                masks=masks,
                camera_model=camera_model_to_use,
            )
    ...
    def eval(self, step: int, stage: str = "val"):
        ...
        for i, idx in enumerate(idxs):
            ...
            image_name = dataset.parser.image_names[idx]
            camera_model_to_use = self._get_camera_model_from_name(image_name)
    ...
    def render_traj(self, step: int, camtoworlds_all=None, tag="novel"):
        ...
        if camtoworlds_all is None:
            is_original_path = (cfg.render_traj_path == "original")
            if is_original_path:
                 camtoworlds_all = self.parser.camtoworlds
            else:
                base_c2w = self.parser.camtoworlds[5:-5]
            if cfg.render_traj_path == "interp":
                camtoworlds_all = generate_interpolated_path(base_c2w, 1)
            elif cfg.render_traj_path == "ellipse":
                height = base_c2w[:, 2, 3].mean()
                camtoworlds_all = generate_ellipse_path_z(base_c2w, height=height)
        ...
        pbar = tqdm.trange(0, len(my_ids), desc=f"Rendering trajectory (rank {rank})",
                           disable=(world > 1 and rank != 0))
        
        for i in pbar:
            fid = my_ids[i]  # Get the original index of the frame
            c2w = camtoworlds_all[fid:fid+1] # Get a single camera pose, keep batch dim
            Ks  = K[None] # Add batch dim to intrinsics

            # 决定相机模型
            # 只有当渲染原始轨迹时，我们才能从parser中获取文件名
            camera_model_to_use = "pinhole" # 默认为pinhole
            if 'is_original_path' in locals() and is_original_path:
                image_name = self.parser.image_names[fid]
                camera_model_to_use = self._get_camera_model_from_name(image_name)
            ...
            renders, alphas, _ = self.rasterize_splats(
                ...
                render_mode="RGB",
                distributed=False,
                camera_model=camera_model_to_use # <-- 传递动态模型
            )

            # 写盘逻辑保持不变，但现在处理的是单帧结果
            colors_u8 = (torch.clamp(renders[0, ..., :3], 0.0, 1.0) * 255).byte().cpu().numpy()
            alphas_u8 = (alphas[0, ..., 0] * 255).byte().cpu().numpy()

            pred_path = f"{pred_dir}/{fid:04d}.{ 'jpg' if cfg.render_format=='jpg' else 'png' }"
            img = Image.fromarray(colors_u8)
            ...
            aimg = Image.fromarray(alphas_u8, mode='L')
