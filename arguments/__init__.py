...
class AuxiliaryParams(ParamGroup):
    def __init__(self, parser, sentinel=False):
        ...
        self.test_iterations = [7_000, 25000, 50_000, 75_000, 100_000]
        self.save_iterations = [7_000, 25000, 50_000, 75_000, 100_000]
        self.quiet = False
        self.checkpoint_iterations = [7_000, 25000, 50_000, 75_000]
        self.start_checkpoint = ""
        self.auto_start_checkpoint = False
        self.log_folder = "/tmp/gaussian_splatting"
        self.log_interval = 250
        self.llffhold = 8
        self.backend = "gsplat" # "default", "gsplat"
        self.scale = 1.0
...
class ModelParams(ParamGroup):
    def __init__(self, parser, sentinel=False):
        ...
        self.eval = False
        self.mcmc = False
        self.init_type = "sfm" # "sfm", "random"
        self.mcmc_noise_scale = 5e5
        self.cap_max = 2000000
        self.init_num_pts = 1000000
        super().__init__(parser, "Loading Parameters", sentinel)
...
class DistributionParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.redistribute_gaussians_mode = "random_redistribute"  # "no_redistribute", "smart_placement"
...
def init_args(args):
    ...
    if args.smart_image_placement:
        assert args.local_sampling, "smart_image_placement works only when local_sampling==True that we distribute dataset storage to each GPU."

    if not args.gaussians_distribution:
        args.distributed_save = False
    
    if args.redistribute_gaussians_mode == "smart_placement":
        assert args.smart_image_placement, "smart_placement works only when smart_image_placement==True"
        assert not args.mcmc, "smart_placement works only when mcmc==False"
