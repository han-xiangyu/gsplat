...
def all_to_all_communication_final(
    ...
    gaussian2gpuid_cnt = torch.zeros(
        (batched_screenspace_params[0][0].shape[0], utils.DEFAULT_GROUP.size()),
        dtype=torch.int8,
        device="cuda",
    )  # NOTE: int8 should be enough for the counter. However, when we have > 256 GPUs, we may need to change the dtype.
    ...
    for k in range(num_cameras):
        ...
        for local_id, global_id in enumerate(strategy.gpu_ids):
            ...
            gaussian2gpuid_cnt[local2j_ids[local_id], global_id] += 1
    ...
    def one_all_to_all(batched_tensors, use_function_version=False):
        ...
        if timers is not None:
            timers.stop("forward_all_to_all_communication")
            timers.start("actual_communication")
        if (
        ...
        if timers is not None:
            timers.stop("actual_communication")
            timers.start("forward_all_to_all_communication")
...
def gsplat_all_to_all_communication_final(
    ...
    gaussian2gpuid_cnt = torch.zeros(
        (batched_means2D[0].shape[0], utils.DEFAULT_GROUP.size()),
        dtype=torch.int8,
        device="cuda",
    )  # NOTE: int8 should be enough for the counter. However, when we have > 256 GPUs, we may need to change the dtype.
...
def distributed_preprocess3dgs_and_all2all_final(
    ...
    batched_screenspace_pkg = {
        ...
        "gaussian2gpuid_cnt": gaussian2gpuid_cnt,
    }
    return batched_screenspace_pkg
